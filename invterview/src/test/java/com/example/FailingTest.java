...
public class FailingTest {
    ...
    public void absoluteLaziness() throws Exception {
        final Iterable<Integer> iterable = getIntegers();
        Iterable<Iterable<Integer>> iterables = MyIterators.partitionWithStreams(iterable, Integer.MAX_VALUE);
        Iterator<Iterable<Integer>> bastard = iterables.iterator();
        bastard.next();
    }
    
    private Iterable<Integer> getIntegers() {
        final Stream<Integer> myInfiniteStream = IntStream.iterate(0, operand -> operand + 1).boxed();
